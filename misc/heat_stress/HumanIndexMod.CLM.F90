...
! !ARGUMENTS:
    ...
    real(r8), intent(in) :: vape 	! Vapour Pressure (Pa)
...
!           Also adding convergence criteria
    real(r8) :: convergence = 0.0001_r8 ! Convergence value
...
! !ARGUMENTS:
    implicit none
    real(r8), intent(in)  :: rh        ! Relative Humidity (%)
    real(r8), intent(in)  :: e         ! Saturated Vapour Pressure (Pa)
    real(r8), intent(out) :: erh       ! Vapour Pressure (Pa)
...
!
    ...
    real(r8) :: es_C = 6.112_r8		! Vapour Pressure at Freezing STD (mb)
